...
This page explains what the schema captures, why it is invariant,
and what behavioural questions it makes answerable.

## The six categories

| Category      | Events                                                                              | What they capture                                              |
|---------------|-------------------------------------------------------------------------------------|----------------------------------------------------------------|
| Session       | `SESSION_START`, `SESSION_END`                                                       | Participant identity plus environment plus lifetime            |
...
Sessions with no impressions contribute nothing. They are not 0/0 =
NaN, and they are not a synthetic 0 that would dilute the average.
...
- `<FeedSurface>` wraps a feed page. It wires page-load, scroll,
  mouse, viewport, and visibility events. It fires `HOME_FEED` for
  newly seen videos.
- `<WatchSurface>` wraps a watch page. Same as `<FeedSurface>`, plus
  `VIDEO_META_CAPTURED` for the current video and `RECOMMENDATIONS`
  for the related list.
- `<VideoSurface>` wraps a single card or player. Card mode emits
  `IMPRESSION`, `THUMBNAIL_HOVER`, and either `FEED_CLICK` or
  `VIDEO_CLICK`. Watch mode produces the playback event family via
  render-prop handlers.

A UI track **must** mount these surfaces. There is no way to render
a feed page through the platform's data hooks without going through
...
- **Position effects.** Did changing the feed layout (for example,
  4-col grid → 1-col mobile list) change which positions get clicked?
- **Algorithm comparisons.** With the policy attached to each event,
  CTR / Watch Ratio / Session Length / Session Duration medians are
...
- **Behaviour patterns.** Mouse, scroll, focus, and tab visibility
  give a signal-rich attention proxy without survey overhead.
- **Self-report alignment.** Surveys (with [three timings, pre /
  post / inter-session](./survey-system)) attach to the same user
  identity, so behavioural and self-report data sit in the same data
  model.
...
- [**Authoring a UI**](../guides/authoring-a-ui). How surfaces enter
  a custom UI without ceremony.
